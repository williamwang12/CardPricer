...
try:
    if "seeded" not in st.session_state:
        n = seed_from_excel("samples/ExistingInventory.xlsx", user_email)
        if n > 0:
            st.toast(f"Imported {n} cards from samples/ExistingInventory.xlsx")
        ...
        if show_logo:
            ...
            if logo_file:
                ...
                with open("../assets/Logo.png", "rb") as f:
                    logo_bytes = f.read()
                st.caption("Using Logo.png from assets folder")
            else:
                logo_bytes = None
        else:
            logo_bytes = None
