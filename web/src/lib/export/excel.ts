...
export async function exportInventory(
  ...
  for (const c of cards) {
    ...
    const price = formatPrice(c.market_price, currency, rate, "ceil", 0);
...
export async function exportPriceList(
  ...
  for (const c of cards) {
    ...
    const price = c.market_price != null
      ? formatPrice(c.market_price, currency, rate, "ceil", 0)
      : "";
    for (let i = 0; i < c.quantity; i++) {
      ws.addRow([label, price]);
    }
...
export async function exportMovers(
  ...
  for (const m of movers) {
    if (m.newPrice != null && m.newPrice >= minPrice) {
      ...
      ws.addRow([label, formatPrice(m.newPrice, currency, rate, "ceil", 0)]);
    }
  }
