...
export async function exportInventory(cards: Card[]): Promise<Buffer> {
  ...
  for (const c of cards) {
    ...
    const price = `$${Math.round(c.market_price)}`;
...
export async function exportPriceList(cards: Card[]): Promise<Buffer> {
  const wb = new ExcelJS.Workbook();
  ...
  for (const c of cards) {
    const label = c.number ? `${c.name} #${c.number}` : c.name;
    const price = c.market_price != null ? `$${Math.ceil(c.market_price)}` : "";
    for (let i = 0; i < c.quantity; i++) {
      ws.addRow([label, price]);
    }
  }
